...
#pragma mark --五星好评
- (void)fiveStarHighPraise:(NSString *)userid sucess:(sucessCompleteBlock)sucessBlock failure:(failureCompleteBlock)failureBlock{
    NSString *sever = UserDefaultObjectForKey(LOTTERY_HOST_NAME_SEVER);
    NSString *url = [NSString stringWithFormat:@"%@/comment_status",sever];
...
- (void)topAdSucess:(sucessCompleteBlock)sucessBlock failure:(failureCompleteBlock)failureBlock{
    NSString *sever = UserDefaultObjectForKey(LOTTERY_HOST_NAME_SEVER);
    NSString *url = [NSString stringWithFormat:@"%@/caipiao/appqun/topad",sever];
...
- (void)getawarddataSucessBlock:(sucessCompleteBlock)sucesBlock failureBlock:(failureCompleteBlock)failureBlock{
    NSString *url = [NSString stringWithFormat:@"%@/api/gdkl10/getawarddata",@"http://m.1396mp.com"];
    [[HRNetworkTools sharedNetworkTools]GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
